...
public class BasicDatastoreCRUDTest extends BasicDatastoreTestBase {
    ...
    public void updateDocument_existingDocument_success() throws Exception {
        ...
        Assert.assertTrue(((BasicDocumentRevision)rev_2).isCurrent()); // new revision is current revision

        BasicDocumentRevision rev_1_again = datastore.getDocument(rev_1.getId(), rev_1.getRevision());
        Assert.assertTrue(((BasicDocumentRevision)rev_1).isCurrent()); // rev_1 is still marked as "current", and developer need query db to get the latest data, yikes :(
    ...
    public void updateDocument_targetDocumentNotCurrentRevision_exception() throws Exception{
        ...
        Assert.assertFalse(((BasicDocumentRevision)rev_1).isCurrent());
        rev_1Mut = rev_1;
        rev_1Mut.setBody(bodyOne);
    ...
    public void deleteDocument_previousRevisionWasWinner_newRevisionInsertedAsWinner()
        ...
        Assert.assertTrue(((BasicDocumentRevision)deletedRev).isCurrent());
    ...
    public void getDocumentsWithInternalIds_twoIds_allSpecifiedDocumentsShouldBeReturnedInCorrectOrder() throws
        ...
        List<Long> ids = new ArrayList<Long>();
        ids.add(((BasicDocumentRevision)dbObjects[1]).getInternalNumericId());
        ids.add(((BasicDocumentRevision)dbObjects[0]).getInternalNumericId());
        ids.add(101L);
    ...
    public void getDocumentsWithInternalIds_moreIdsThanSQLiteParameterLimit() throws Exception {
        ...
        for (int i = 0; i < n_docs; i++) {
            ...
            internal_ids.add(((BasicDocumentRevision)saved).getInternalNumericId());
        }
        ...
        for (int[] test : tests) {
            ...
            List<? extends DocumentRevision> docs = datastore.getDocumentsWithInternalIds(
    ...
    private void getAllDocuments_testCountAndOffset(int objectCount, List<DocumentRevision> expectedDocumentRevisions, boolean descending) {
        ...
        int count;
        int offset = 0;
        List<? extends DocumentRevision> result;
    ...
    private void getAllDocuments_compareResult(List<DocumentRevision> expectedDocumentRevisions, List<? extends DocumentRevision> result, int count, int offset) {
        ListIterator<? extends DocumentRevision> iterator;
